...
void LCD_Scan_Dir(u8 dir)	//设置屏扫描方向,扫描方向不同，屏幕坐标系会随之变化
{
	...
	LCD_write_reg(0X2A00+2);	LCD_write_ram((width-1)>>8); 
	LCD_write_reg(0X2A00+3);	LCD_write_ram((width-1)&0XFF); 
	LCD_write_reg(0X2B00)  ;  	LCD_write_ram(0); 
	LCD_write_reg(0X2B00+1);	LCD_write_ram(0); 
	LCD_write_reg(0X2B00+2);	LCD_write_ram((height-1)>>8); 
	LCD_write_reg(0X2B00+3);	LCD_write_ram((height-1)&0XFF); 
